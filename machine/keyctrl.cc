...
/* Hier muesst ihr selbst Code vervollstaendigen */   
	...
	//testen ob etwas im Ausgabepuffer
    do
    { status = ctrl_port.inb ();      
    } while ((status & outb) == 0);
	if((status & auxb) != 0){ return invalid;} 

	//make-/breakcode auslesen und testen ob code zu einem Zeichen decodiert werden kann
	code = data_port.inb();
	if(key_decoded()){
		return gather;
	}
...
void Keyboard_Controller::set_repeat_rate (int speed, int delay)
 {
	...
	switch(speed){
		case 25: temp |= 0x02; break;
		case 20: temp |= 0x04; break;
		case 15: temp |= 0x08; break;
		case 10: temp |= 0x0c; break;
		case 7: temp |= 0x10; break;
		case 5: temp |= 0x14; break;
		default: break;
	}
	//delay: 0: 0.25sec; 1: 0.5sec; 2: 0.75sec; >=3: 1sec; 
	switch(delay){
		case 0: break;
		case 1: temp |= 0x100; break;
		case 2: temp |= 0x200; break;
		default: temp |= 0x300; break;
	}
	...
	data_port.outb(temp); 
		...
		do
   	{ status = ctrl_port.inb (); // data_port ist I/O port. einlesen in status      
    } while ((status & inpb) != 0);
...
void Keyboard_Controller::set_led (char led, bool on)
 {	
	...
	/* inpb ist jetzt gesetzt, es gibt ein Zeichen, das die CPU gesetzt hat, aber das der Tastaturcontroller noch nicht abgeholt hat */

	/* Ausgabe über den Ausgabepuffer des Tastaturcontrollers */
	data_port.outb(kbd_cmd::set_led);
	
	/* warte auf ACK */	
 	/* prüfe, ob im Ausgabepuffer das Bestätigungsbyte 0xfa (ACK) enthalten ist */
	...
	/* falls ACK enthalten, sende LED daten an tastatur. */
	data_port.outb(leds); 
